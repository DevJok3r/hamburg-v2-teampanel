...
const MILESTONES = [
  { p: 0,   label: 'Bestätigt' },
  { p: 25,  label: 'Bearbeitung' },
  { p: 50,  label: 'Fortschritt' },
  { p: 75,  label: 'Fast fertig' },
  { p: 100, label: 'Fertig' },
];
          ...
          <div className="bg-[#1a1d27] border border-white/[0.07] rounded-3xl overflow-hidden">
                      ...
                      return (
                        <div key={m.p} className="flex flex-col items-center gap-1.5">
                          <div className={`w-6 h-6 rounded-full border flex items-center justify-center text-xs transition-all
                            ${done ? 'bg-gradient-to-br from-blue-500 to-violet-500 border-blue-500 text-white' : 'bg-white/5 border-white/[0.08] text-gray-700'}`}>
                            {done ? '✓' : '·'}
                          </div>
            ...
            {order.notes && (
              <div className="mx-6 mb-6 bg-blue-950/20 border border-blue-500/10 rounded-2xl p-4">
                <p className="text-blue-400 text-xs font-medium mb-1">📋 Hinweis</p>
