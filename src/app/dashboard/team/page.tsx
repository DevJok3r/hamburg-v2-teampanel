...
export default function TeamPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase.from('profiles').select('*').eq('is_active', true).order('role').order('username');
    setMembers(data || []);
    setLoading(false);
  }
  ...
  async function loadMemberDetails(memberId: string) {
    const { data: memberEntries } = await supabase
      .from('member_entries').select('*, creator:created_by(username)')
      .eq('user_id', memberId).order('created_at', { ascending: false });
    const { data: memberWarnings } = await supabase
      .from('warnings').select('*, creator:created_by(username)')
      .eq('user_id', memberId).order('created_at', { ascending: false });
  ...
  async function openMember(member: Profile) {
    ...
    setShowEntryForm(false); setShowWarnForm(false);
    setShowKickConfirm(false); setShowRoleForm(false); setShowAbsenceForm(false);
    setSelectedRole(member.role);
    if (myRole && can.manageEntries(myRole)) await loadMemberDetails(member.id);
  }

  async function addEntry() {
    if (!entryForm.text.trim() || !selectedMember) return;
    await supabase.from('member_entries').insert({ user_id: selectedMember.id, type: entryForm.type, text: entryForm.text, created_by: myId });
  ...
  async function addWarning() {
    ...
    await supabase.from('warnings').insert({ user_id: selectedMember.id, reason: warnForm.reason, created_by: myId });
    setWarnForm({ reason: '' });
  ...
  async function kickMember() {
    ...
    const { error } = await supabase.from('profiles').update({ is_active: false }).eq('id', selectedMember.id);
  ...
  async function changeRole() {
    ...
    if (myRole !== 'top_management' && ROLE_HIERARCHY[selectedRole] >= ROLE_HIERARCHY[myRole]) return;
    await supabase.from('profiles').update({ role: selectedRole }).eq('id', selectedMember.id);
  ...
  async function addAbsence() {
    ...
    await supabase.from('absences').insert({ user_id: selectedMember.id, from_date: absenceForm.from_date, to_date: absenceForm.to_date, reason: absenceForm.reason });
    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1d27] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              ...
              <button onClick={() => setSelectedMember(null)} className="text-gray-400 hover:text-white transition">
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-[#0f1117] rounded-lg p-3">
                  ...
                  <p className="text-white text-sm">{selectedMember.last_sign_in_at ? new Date(selectedMember.last_sign_in_at).toLocaleDateString('de-DE') : 'Noch nie'}</p>
                </div>
                <div className="bg-[#0f1117] rounded-lg p-3">
                  <p className="text-gray-400 text-xs mb-1">Verwarnungen</p>
                  <p className={`text-sm font-bold ${warnings.length > 0 ? 'text-red-400' : 'text-green-400'}`}>{warnings.length}</p>
                </div>
                <div className="bg-[#0f1117] rounded-lg p-3">
              ...
              {canManage && selectedMember.id !== myId && canActOn(selectedMember.role) && (
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => { setShowEntryForm(!showEntryForm); setShowWarnForm(false); setShowKickConfirm(false); setShowRoleForm(false); setShowAbsenceForm(false); }}
                  ...
                  <button onClick={() => { setShowWarnForm(!showWarnForm); setShowEntryForm(false); setShowKickConfirm(false); setShowRoleForm(false); setShowAbsenceForm(false); }}
                    className="bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 text-sm font-medium px-4 py-2 rounded-lg transition">
                    Verwarnen
                  </button>
                  {canChangeRole && (
                    <button onClick={() => { setShowRoleForm(!showRoleForm); setShowEntryForm(false); setShowWarnForm(false); setShowKickConfirm(false); setShowAbsenceForm(false); }}
                      className="bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/30 text-sm font-medium px-4 py-2 rounded-lg transition">
                      Rolle ändern
                    </button>
                  )}
                  <button onClick={() => { setShowAbsenceForm(!showAbsenceForm); setShowEntryForm(false); setShowWarnForm(false); setShowKickConfirm(false); setShowRoleForm(false); }}
                    className="bg-orange-500/10 hover:bg-orange-500/20 text-orange-400 border border-orange-500/30 text-sm font-medium px-4 py-2 rounded-lg transition">
                    Abmelden
                  </button>
                  {canKick && (
                    <button onClick={() => { setShowKickConfirm(!showKickConfirm); setShowEntryForm(false); setShowWarnForm(false); setShowRoleForm(false); setShowAbsenceForm(false); }}
                      className="bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 text-sm font-medium px-4 py-2 rounded-lg transition">
                      Rauswerfen
                    </button>
                  )}
                </div>
              ...
              {showRoleForm && (
                <div className="bg-[#0f1117] rounded-xl p-4 space-y-3">
                  ...
                  <select value={selectedRole} onChange={e => setSelectedRole(e.target.value as UserRole)}
                    className="w-full bg-[#1a1d27] border border-white/10 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500">
                    {allRoles.filter(r => {
                      if (myRole === 'top_management') return true;
                      return ROLE_HIERARCHY[r] < (ROLE_HIERARCHY[myRole!] ?? 0);
                    }).map(r => (
                  ...
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowRoleForm(false)}
                      className="bg-white/5 hover:bg-white/10 text-gray-300 text-sm px-4 py-2 rounded-lg transition">Abbrechen</button>
                    <button onClick={changeRole}
                      className="bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">Speichern</button>
                  </div>
                </div>
              )}
              ...
              {showAbsenceForm && (
                <div className="bg-[#0f1117] rounded-xl p-4 space-y-3">
                  ...
                  <textarea value={absenceForm.reason} onChange={e => setAbsenceForm(p => ({ ...p, reason: e.target.value }))}
                    placeholder="Grund der Abmeldung..." rows={2}
                    className="w-full bg-[#1a1d27] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 text-sm focus:outline-none focus:border-blue-500 resize-none" />
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowAbsenceForm(false)}
                      className="bg-white/5 hover:bg-white/10 text-gray-300 text-sm px-4 py-2 rounded-lg transition">Abbrechen</button>
                    <button onClick={addAbsence}
                      className="bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">Abmelden</button>
                  </div>
                </div>
              )}
                ...
                <div className="bg-[#0f1117] rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    {(['misconduct', 'positive', 'other'] as EntryType[]).map(t => (
                      <button key={t} onClick={() => setEntryForm(p => ({ ...p, type: t }))}
                  ...
                  <textarea value={entryForm.text} onChange={e => setEntryForm(p => ({ ...p, text: e.target.value }))}
                    placeholder="Beschreibung des Eintrags..." rows={3}
                    className="w-full bg-[#1a1d27] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 text-sm focus:outline-none focus:border-blue-500 resize-none" />
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowEntryForm(false)}
                      className="bg-white/5 hover:bg-white/10 text-gray-300 text-sm px-4 py-2 rounded-lg transition">Abbrechen</button>
                    <button onClick={addEntry}
                      className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">Speichern</button>
                  </div>
                </div>
              )}
              ...
              {showWarnForm && (
                <div className="bg-[#0f1117] rounded-xl p-4 space-y-3">
                  ...
                  <textarea value={warnForm.reason} onChange={e => setWarnForm({ reason: e.target.value })}
                    placeholder="Grund der Verwarnung..." rows={3}
                    className="w-full bg-[#1a1d27] border border-white/10 rounded-lg px-4 py-2.5 text-white placeholder-gray-500 text-sm focus:outline-none focus:border-blue-500 resize-none" />
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setShowWarnForm(false)}
                      className="bg-white/5 hover:bg-white/10 text-gray-300 text-sm px-4 py-2 rounded-lg transition">Abbrechen</button>
                    <button onClick={addWarning}
                      className="bg-yellow-600 hover:bg-yellow-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">Verwarnen</button>
                  </div>
                </div>
              )}
                ...
                <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={kickMember}
                      className="bg-red-600 hover:bg-red-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">Ja, rauswerfen</button>
                  </div>
                </div>
              )}
                    ...
                    {warnings.map(w => (
                      <div key={w.id} className="bg-[#0f1117] rounded-lg p-3 flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-gray-500 text-xs mt-1">von {(w.creator as any)?.username} · {new Date(w.created_at).toLocaleDateString('de-DE')}</p>
                        </div>
                    ...
                    {entries.map(e => (
                      <div key={e.id} className="bg-[#0f1117] rounded-lg p-3 flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-gray-500 text-xs mt-1">von {(e.creator as any)?.username} · {new Date(e.created_at).toLocaleDateString('de-DE')}</p>
                        </div>
                        {myRole && can.deleteEntries(myRole) && (
              ...
              {canManage && entries.length === 0 && warnings.length === 0 && (
                <p className="text-gray-500 text-sm text-center py-4">Keine Einträge oder Verwarnungen vorhanden</p>
              )}
            </div>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {members.map(member => (
            <div key={member.id} onClick={() => openMember(member)}
              ...
              <div className="text-xs text-gray-500">Dabei seit: {new Date(member.created_at).toLocaleDateString('de-DE')}</div>
              {canManage && <p className="text-blue-400 text-xs mt-2">Klicken für Details →</p>}
            </div>
          ))}
        </div>
      )}
