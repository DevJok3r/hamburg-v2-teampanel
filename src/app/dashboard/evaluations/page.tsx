...
type FormState = {
  user_id: string; period: string; notes: string;
  score_activity: number | null; score_player_interaction: number | null;
  score_task_completion: number | null; score_resilience: number | null;
  score_problem_solving: number | null; score_player_communication: number | null;
  score_team_communication: number | null; score_initiative: number | null;
  score_reliability: number | null; score_rule_knowledge: number | null;
};
...
// ─── EvaluationForm außerhalb der Hauptkomponente ────────────────────────────
function EvaluationForm({ form, setForm, onSave, onCancel, members, isEdit }: {
  form: FormState;
  ...
  const scores = CATEGORIES.map(c => form[c.key as keyof FormState] as number | null).filter(s => s !== null) as number[];
    ...
    <div className="space-y-5">
        ...
        {CATEGORIES.map(cat => (
          <div key={cat.key} className="bg-[#0f1117] rounded-lg p-3">
            <div className="flex items-center justify-between gap-4">
              ...
              <ScoreSelector
                value={form[cat.key as keyof FormState] as number | null}
      ...
      <div>
        ...
        <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
          placeholder="Zusätzliche Anmerkungen zur Bewertung..." rows={3}
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button onClick={() => onSave('draft')}
          disabled={!form.user_id || !form.period.trim()}
          className="bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 disabled:opacity-40 font-medium px-4 py-2 rounded-lg text-sm transition">
          Als Entwurf speichern
        </button>
        <button onClick={() => onSave('final')}
          disabled={!form.user_id || !form.period.trim()}
          className="bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white font-medium px-5 py-2 rounded-lg text-sm transition">
          Abschließen
        </button>
      </div>
    </div>
...
export default function EvaluationsPage() {
  ...
  const [activeTab, setActiveTab]     = useState<'all' | 'draft' | 'final'>('all');
  ...
  async function saveEvaluation(status: 'draft' | 'final') {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(true); setEditEval(null); setForm({ ...EMPTY_FORM }); }}
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-[#1a1d27] border border-green-500/20 rounded-xl p-4 text-center">
          <p className="text-2xl font-bold text-green-400">{counts.final}</p>
      ...
      <div className="flex gap-2">
        {([
          ...
          { key: 'final', label: `Abgeschlossen (${counts.final})` },
        ] as { key: 'all' | 'draft' | 'final'; label: string }[]).map(tab => (
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1d27] border border-white/10 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <EvaluationForm
              ...
              onCancel={() => { setShowForm(false); setEditEval(null); setForm({ ...EMPTY_FORM }); }}
              members={members}
              isEdit={!!editEval}
            />
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-[#1a1d27] border border-white/10 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-white font-bold text-lg">Bewertung – {viewEval.profiles?.username}</h2>
            ...
            {(() => {
              ...
              return (
                <div className="bg-[#0f1117] rounded-xl p-5 text-center mb-5">
                  ...
                  <p className={`text-4xl font-bold ${grade.color}`}>{viewEval.average_score?.toFixed(2) || '–'}</p>
                  <p className={`text-lg font-medium mt-1 ${grade.color}`}>{grade.label}</p>
                  {grade.uprank && <p className="text-gray-400 text-sm mt-1">{grade.uprank}</p>}
                </div>
              );
            })()}
            ...
            <p className="text-gray-500 text-xs">
              Bewertet von {(viewEval.evaluator as any)?.username} · {new Date(viewEval.created_at).toLocaleString('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded border ${ev.status === 'draft' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/30' : 'bg-green-500/10 text-green-400 border-green-500/30'}`}>
                   ...
                   {(ev.status === 'draft' || myRole === 'top_management') && (
                    <>
                      <button onClick={() => {
                        setEditEval(ev);
                        setForm({
                          user_id: ev.user_id, period: ev.period, notes: ev.notes || '',
