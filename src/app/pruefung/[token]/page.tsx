...
function OpenQuestion({ questionId, value, onChange, onBlur }: {
  questionId: string;
  value: string;
  ...
  function handleAnswerChange(id: string, val: string) {
    setAnswers(p => {
      ...
      saveTimer.current = setTimeout(() => {
        supabase.from('exam_sessions').update({ written_answers: updated }).eq('token', token);
      }, 1000);
      return updated;
    });
  }
  ...
  if (error) return (
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-white font-bold text-xl">Prüfungslink ungültig oder abgelaufen.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center">
      <div className="text-center max-w-md px-6">
        ...
        <div className="mt-6 bg-[#1a1d27] border border-white/10 rounded-xl p-4">
          <p className="text-gray-400 text-xs">Du kannst dieses Fenster schließen.</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f1117]">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-4">
        ...
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4">
          <p className="text-yellow-400 text-sm">ℹ️ Beantworte alle Fragen ausführlich. Deine Antworten werden automatisch gespeichert.</p>
        </div>
